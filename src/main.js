...
class BlogAutomation {
    ...
    async executePhotoPublish(payload) {
        try {
            ...
            const { naverId, naverPassword, blogId, claudeApi, images, context, openType, useBubble, useDescription } = payload;
            
            let activeClaudeApi = claudeApi;
            if (this.configManager) {
                ...
                if (localAccount && localAccount.claudeApi) {
                    if (!activeClaudeApi || knownExpiredKeys.includes(activeClaudeApi)) {
                        console.log(`🔑 [서버 키 자동 대체] 모바일 수신 키 대신 PC 서버의 API 키를 적용합니다.`);
                        activeClaudeApi = localAccount.claudeApi;
                    }
                }
            }
            
            if (!naverId || !naverPassword || !blogId || !activeClaudeApi) {
                return {
                    success: false,
            ...
            generator.setApiKey(activeClaudeApi);
...
JSON 외에 다른 여담이나 설명 문구, 백틱(\`\`\`json 등)은 붙이지 말고 순수 JSON 데이터만 출력해주세요.`;
            ...
            // 멀티모달 메시지 파트 구성
            const messageParts = [...imageParts, { type: "text", text: prompt }];
            
            console.log('🤖 Gemini AI 멀티모달 호출...');
            let responseText = await generator.sendMessageWithRetry(messageParts);
            console.log('🤖 Gemini AI 응답 수신 완료');
    ...
    async publishToBlog(productResult, imageResult, account) {
        ...
        try {
            ...
            const blogConfig = {
                ...
                OPEN_TYPE: 2,
                claudeApi: account.claudeApi // Claude API 키 추가
            };
            
            this.blogPublisher = new (require('./modules/BlogPublisher'))(blogConfig);
            console.log(`📂 카테고리 ID: ${account.categoryId || 1}, 블로그 ID: ${account.blogId || account.username}`);
            console.log(`🤖 Claude API 키: ${account.claudeApi ? '설정됨' : '없음'}`);
    ...
    async testClaudeAPI(apiKey) {
        try {
            ...
            if (testResult.success) {
                // API 키 저장
                await this.configManager.setClaudeApiKey(apiKey);
                console.log('✅ Claude API 테스트 성공');
            } else {
                console.error('❌ Claude API 테스트 실패:', testResult.error);
            }
    ...
    try {
        ...
        switch (action) {
            ...
            case 'photo-publish':
                ...
                if (!payload || !payload.naverId || !payload.naverPassword || !payload.blogId || !payload.claudeApi) {
...
// 기존 IPC 핸들러들 복구
ipcMain.handle('test-claude-api', async (event, apiKey) => {
    try {
        if (!blogAutomation) {
            return { success: false, error: 'BlogAutomation 시스템이 초기화되지 않았습니다.' };
        }
        return await blogAutomation.testClaudeAPI(apiKey);
    } catch (error) {
        console.error('❌ IPC testClaudeAPI 오류:', error);
        return { success: false, error: error.message };
    }
});
